...
const categories = computed(() => {
	const selectedStartGroupName = selectedStartGroup.value?.name
	...
	const result = selectedStartGroup.value?.categories.map(
		(category) => selectedStartGroupName + ' ' + category.name
	)
...
const initFilters = () => {
	filters.value = {
		...
		full_category: { value: null, matchMode: FilterMatchMode.CONTAINS },
	}
}
const filters = ref({
	global: { value: null, matchMode: FilterMatchMode.CONTAINS },
	...
	<div>
		...
		<div class="flex flex-col gap-4 mt-9">
			<Card class="border border-primary">
				...
				<template #content>
					<DataTable v-model:filters="filters" :value="liveRanking" dataKey="id" filterDisplay="menu"
						class="text-sm" :globalFilterFields="[
							'full_name',
							'start_group_name',
							'category_name',
						]">
						<template #header>
							<div class="flex flex-col md:flex-row gap-2 mb-4">
								...
								<Select v-model="selectedStartGroup" :options="event.start_groups" optionLabel="name"
									placeholder="Seleccionar grupo" />
								<Select v-model="filters['full_category'].value" :options="categories"
									placeholder="Seleccionar categoría" />
								<Button type="button" icon="pi pi-filter-slash" label="Limpiar" variant="outlined"
									@click="clearFilter()" />
							</div>
						</template>
						...
						<Column field="full_name" header="Nombre completo" style="min-width: 14rem">
							<template #body="{ data }">
								{{ data.full_name }}
							</template>
						...
						<Column field="full_category" header="Categoría">
							<template #body="{ data }">
								<div class="flex flex-col xl:flex-row xl:gap-1">
									<span>{{ data.full_category }}</span>
								</div>
							</template>
							<template #filter="{ filterModel, filterCallback }">
								<InputText v-model="filterModel.value" type="text" @input="filterCallback()"
						...
						<Column field="position_in_category" header="Categoría">
							<template #body="{ data }">
								...
								<i :class="`${data.position_in_category === 1 ||
						...
						<Column field="position_overall" header="General">
							<template #body="{ data }">
								...
								<i :class="`${data.position_overall === 1 ||
